...
export default function ShadowSkylerBluePMO() {
  const [active, setActive] = useState(false);
  return (
    <div className="space-y-5">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-black">PMO Services</h1>
          <p className="text-sm text-muted-foreground">Project Management Office services with governance, methodology, and portfolio management</p>
        </div>
        <Badge className="bg-indigo-600 text-white shrink-0">PMO</Badge>
      </div>
      <div className="grid grid-cols-4 gap-2">
        <Card className="border-border/50 text-center"><CardContent className="py-3 px-2"><p className="font-black text-lg text-violet-400">8247</p><p className="text-xs text-muted-foreground">Projects Managed</p></CardContent></Card>
        <Card className="border-border/50 text-center"><CardContent className="py-3 px-2"><p className="font-black text-lg text-green-400">94pct</p><p className="text-xs text-muted-foreground">On-Time Delivery</p></CardContent></Card>
        <Card className="border-border/50 text-center"><CardContent className="py-3 px-2"><p className="font-black text-lg text-blue-400">94pct</p><p className="text-xs text-muted-foreground">On-Budget</p></CardContent></Card>
      ...
      <div className="rounded-xl bg-gradient-to-br from-indigo-900/40 to-violet-900/40 border border-indigo-500/30 p-4">
        <p className="font-black text-sm mb-1">PMO Services — Activate Now</p>
        ...
        <div className="grid grid-cols-2 gap-2">
          <Button
            ...
            onClick={() => { setActive(true); toast.success("PMO Services activated!"); }}
          >
            <Zap className="h-4 w-4 mr-2" />{active ? "Active" : "Activate"}
          </Button>
          <Button variant="outline" onClick={() => toast.info("Opening PMO Services docs...")}>
