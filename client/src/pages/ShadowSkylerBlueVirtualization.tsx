...
import { Server, Layers, TrendingUp, Shield, ClipboardCheck, Lock, Coins, Cloud, Droplets, DollarSign, CheckCircle, Zap } from "lucide-react";
...
const DATA: Record<string, { icon: string; title: string; desc: string; stats: {label:string;value:string;color:string}[]; features: string[]; accent: string; progress?: {label:string;value:number;color:string}[] }> = {
  ShadowSkylerBlueDataCenter: {
    icon: "🏗️", title: "Data Center Services", accent: "text-slate-400",
    ...
    features: ["Tier III colocation","Managed hosting","Remote hands","Power redundancy","Physical security","24/7 NOC"],
    progress: [
  ...
  ShadowSkylerBlueVirtualization: {
    icon: "⚙️", title: "Virtualization Services", accent: "text-purple-400",
    ...
    features: ["VMware vSphere","Hyper-V/Proxmox","vSAN storage","High availability","Live migration","Capacity planning"],
    progress: [
  ...
  ShadowCryptoDerivatives: {
    icon: "📊", title: "Crypto Derivatives Hub", accent: "text-orange-400",
    ...
    features: ["Perpetual futures","Options chains","Structured products","Delta hedging","Portfolio margin","Greeks analytics"],
    progress: [
  ...
  ShadowCryptoInsurance2: {
    icon: "🛡️", title: "Crypto Insurance 2.0", accent: "text-green-400",
    ...
    features: ["Smart contract coverage","Exchange hack protection","Stablecoin depeg","Wallet theft","DeFi protocol failure","Instant claims"],
    progress: [
  ...
  ShadowSkylerBlueCompliance2: {
    icon: "📋", title: "Compliance Automation", accent: "text-yellow-400",
    ...
    features: ["HIPAA/PCI/SOC2/NIST","Continuous monitoring","Auto evidence collection","Policy management","Risk register","Audit-ready reports"],
    progress: [
  ...
  ShadowSkylerBlueDLP: {
    icon: "🔏", title: "Data Loss Prevention", accent: "text-red-400",
    ...
    features: ["Email DLP","Endpoint DLP","Cloud DLP","PII detection","Credit card detection","Incident reporting"],
    progress: [
  ...
  ShadowCryptoStakingPools: {
    icon: "🏊", title: "Staking Pools", accent: "text-cyan-400",
    ...
    features: ["ETH liquid staking","SKY4444 staking pools","SOL delegation","Auto-compound","Instant unstaking","Validator selection"],
    progress: [
  ...
  ShadowSkylerBlueHybridCloud: {
    icon: "🌤️", title: "Hybrid Cloud Solutions", accent: "text-sky-400",
    ...
    features: ["AWS/Azure/GCP hybrid","SD-WAN connectivity","Workload migration","Unified management","Cost optimization","Disaster recovery"],
    progress: [
  ...
  ShadowCryptoLiquidityMining: {
    icon: "⛏️", title: "Liquidity Mining Hub", accent: "text-amber-400",
    ...
    features: ["LP token mining","Boosted rewards","Auto-harvest","IL protection","Dual rewards","Governance voting"],
    progress: [
  ...
  ShadowSkylerBlueITBudget: {
    icon: "💰", title: "IT Budget Planning", accent: "text-emerald-400",
    ...
    features: ["3-year budget modeling","Vendor negotiation","License optimization","CapEx vs OpEx planning","Cost allocation","Executive reporting"],
    progress: [
...
export default function ShadowSkylerBlueVirtualization() {
  const d = DATA["ShadowSkylerBlueVirtualization"];
  return (
      ...
      {d.progress && (
        <Card className="border-border/50">
          <CardContent className="py-3 px-4 space-y-2">
            {d.progress.map((p: {label:string;value:number;color:string}) => (
              <div key={p.label}>
