...
import { DollarSign, Shield, Wifi, Camera, Database, Phone, CheckCircle, Zap, Users, BarChart3, AlertTriangle, RefreshCw } from "lucide-react";
import { Card, CardContent } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { toast } from "sonner";

export default function ShadowSkylerBlueWiFi() {
  const [active, setActive] = useState(false);
  const title = "ShadowSkylerBlueWiFi".replace(/([A-Z])/g, ' $1').trim();
  const descriptions: Record<string, { icon: string; desc: string; stats: {label:string;value:string;color:string}[]; features: string[] }> = {
    ShadowCryptoPayroll: {
      icon: "💰",
      ...
      features: ["Multi-currency payroll","Auto tax withholding","Crypto-to-fiat conversion","Pay stubs & W-2s","Direct deposit","SKY4444 bonus programs"],
    },
    ShadowSkylerBlueDR: {
      icon: "🔄",
      ...
      features: ["Automated daily backups","Offsite replication","Instant failover","Ransomware protection","Compliance reporting","24/7 monitoring"],
    },
    ShadowSkylerBlueWiFi: {
      ...
      features: ["Unifi/Meraki management","Guest network isolation","Bandwidth throttling","Rogue AP detection","Coverage heat maps","Remote troubleshooting"],
    },
    ShadowSkylerBlueCCTV: {
      ...
      features: ["HD/4K cameras","AI motion detection","License plate recognition","Cloud recording","Mobile viewing","Instant alerts"],
    },
    ShadowSkylerBlueERP: {
      icon: "🏢",
      ...
      features: ["Inventory management","Accounts payable/receivable","HR & payroll","CRM integration","Custom reporting","API integrations"],
    },
    ShadowSkylerBluePBX: {
      icon: "☎️",
      ...
      features: ["3CX/Asterisk/FreePBX","Auto-attendant IVR","Call recording","Ring groups","Voicemail to email","SIP trunk integration"],
    },
  };
  const d = descriptions["ShadowSkylerBlueWiFi"];
  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-2xl font-black">{d.icon} {title.replace("Shadow","").replace("Skyler Blue","Skyler Blue ").trim()}</h1>
      ...
      <div className="rounded-xl bg-muted/50 border border-border/50 p-4 text-center">
        <p className="font-bold text-sm">Get started with Skyler Blue IT Resolutions</p>
        <p className="text-2xl font-black text-indigo-400 mt-1">479-406-7123</p>
        <p className="text-xs text-muted-foreground mb-3">skylerblue4444@gmail.com · Free consultation</p>
        <Button className="w-full h-10 bg-indigo-600 text-white border-0 font-black" onClick={() => toast.success("Consultation request sent — we'll call 479-406-7123 within 1 business hour")}>
          <Zap className="h-4 w-4 mr-2" />Request Free Consultation
        </Button>
      </div>
    </div>
  );
