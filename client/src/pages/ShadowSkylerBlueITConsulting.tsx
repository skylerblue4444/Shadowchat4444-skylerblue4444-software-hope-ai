...
import { Building2, Brain, Lock, Zap, GraduationCap, Coins, Briefcase, Rocket, Shield, Gamepad2, Headphones, Star, CheckCircle, TrendingUp, Users } from "lucide-react";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { toast } from "sonner";

const DATA: Record<string, { icon: string; title: string; desc: string; stats: {label:string;value:string;color:string}[]; features: string[]; accent: string; highlight?: string }> = {
  ShadowSkylerBlueMSP3: {
    icon: "🏆", title: "MSP Platinum Tier", accent: "text-yellow-400",
    ...
    features: ["Dedicated CIO/vCISO","Unlimited support","On-site engineers","24/7/365 coverage","Strategic IT planning","Custom SLA"],
  },
  ShadowCryptoSentiment: {
    icon: "🧠", title: "Crypto Sentiment AI", accent: "text-violet-400",
    ...
    features: ["Twitter/Reddit/Telegram","News sentiment","Fear & Greed index","Whale wallet tracking","On-chain signals","Price prediction"],
  },
  ShadowSkylerBlueZeroTrust: {
    icon: "🔐", title: "Zero Trust Security", accent: "text-red-400",
    ...
    features: ["Identity verification","Micro-segmentation","Continuous validation","MFA everywhere","Privileged access mgmt","Lateral movement prevention"],
  },
  ShadowCryptoFlashLoans: {
    icon: "⚡", title: "Flash Loan Center", accent: "text-yellow-400",
    ...
    features: ["Uncollateralized loans","Single-tx execution","Arbitrage templates","Liquidation bots","Collateral swap","Custom strategies"],
  },
  ShadowSkylerBlueITTraining: {
    icon: "🎓", title: "IT Training & Certification", accent: "text-blue-400",
    ...
    features: ["CompTIA A+/Network+/Security+","Microsoft certifications","Cisco CCNA/CCNP","Cybersecurity training","Hands-on labs","Exam vouchers"],
  },
  ShadowCryptoNFTFi: {
    icon: "🖼️", title: "NFTFi — NFT Finance", accent: "text-pink-400",
    highlight: "New",
    ...
    features: ["NFT-backed loans","Buy now pay later","NFT rental","Fractionalization","Collection floor loans","Instant liquidity"],
  },
  ShadowSkylerBlueITConsulting: {
    icon: "💼", title: "IT Consulting", accent: "text-indigo-400",
    highlight: "Strategic",
    desc: "Strategic IT consulting for Arkansas businesses — technology assessments, digital transformation, and IT strategy",
    stats: [
      { label: "Consulting Engagements",value: "247",color: "text-indigo-400"},
    ...
    features: ["Technology assessment","Digital transformation","IT strategy roadmap","Vendor selection","Change management","Project management"],
  },
  ShadowCryptoSkyBlueICO: {
    icon: "🚀", title: "SKY4444 ICO Dashboard", accent: "text-cyan-400",
    highlight: "Live Now",
    ...
    stats: [
      ...
      { label: "Token Price",        value: "/usr/bin/bash.047",color: "text-green-400"  },
      { label: "Days Left",          value: "47",    color: "text-orange-400" },
    ],
    features: ["3 investment tiers","Vesting schedule","Whitelist priority","Referral bonuses","DAO governance rights","Staking rewards"],
  },
  ShadowSkylerBlueManagedSOC: {
    icon: "🛡️", title: "Managed SOC", accent: "text-red-400",
    highlight: "24/7",
    ...
    features: ["24/7 SOC analysts","SIEM management","Threat hunting","Incident response","Forensic investigation","Monthly SOC reports"],
  },
  ShadowCryptoWeb3Gaming: {
    icon: "🎮", title: "Web3 Gaming Hub", accent: "text-purple-400",
    ...
    features: ["Play-to-earn mechanics","NFT game items","Tournament system","Guild management","Scholarship program","Cross-game assets"],
  },
  ShadowSkylerBlueTechSupport: {
    icon: "🔧", title: "Tech Support Center", accent: "text-orange-400",
    ...
    features: ["Computer repair","Virus/malware removal","Data recovery","Network setup","Software installation","Remote support"],
  },
  ShadowCryptoSkyBlueToken: {
    icon: "💎", title: "SKY4444 Token Hub", accent: "text-sky-400",
    ...
    features: ["Platform utility token","Governance voting","Staking rewards","Fee discounts","ICO participation","Charity allocation"],
  },
};

export default function ShadowSkylerBlueITConsulting() {
  const d = DATA["ShadowSkylerBlueITConsulting"];
  return (
    <div className="space-y-5">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-black">{d.icon} {d.title}</h1>
          <p className="text-sm text-muted-foreground">{d.desc}</p>
        </div>
        {d.highlight && <Badge className="bg-indigo-600 text-white shrink-0">{d.highlight}</Badge>}
      </div>
          ...
          <div className="grid grid-cols-2 gap-1.5">
            {d.features.map((f: string) => (
              <div key={f} className="flex items-center gap-1.5 text-xs text-muted-foreground">
                <CheckCircle className="h-3 w-3 text-green-400 shrink-0" />{f}
              </div>
            ))}
          </div>
        </CardContent>
      ...
      <div className="rounded-xl bg-muted/50 border border-border/50 p-4 text-center">
        ...
        <p className="text-xs text-muted-foreground mb-3">skylerblue4444@gmail.com</p>
        <Button className="w-full h-10 bg-indigo-600 text-white border-0 font-black" onClick={() => toast.success("Request submitted — we'll contact you within 1 hour!")}>
