...
export default function ShadowSocialGroups() {
  const [active, setActive] = useState(false);
  return (
    <div className="space-y-5">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-black">Social Groups</h1>
          <p className="text-sm text-muted-foreground">Create and join crypto communities, trading groups, and interest-based social circles</p>
        </div>
        <Badge className="bg-indigo-600 text-white shrink-0">Find Your Tribe</Badge>
      </div>
      <div className="grid grid-cols-4 gap-2">
        ...
        <Card className="border-border/50 text-center"><CardContent className="py-3 px-2"><p className="font-black text-lg text-green-400">84M</p><p className="text-xs text-muted-foreground">Members</p></CardContent></Card>
        ...
        <Card className="border-border/50 text-center"><CardContent className="py-3 px-2"><p className="font-black text-lg text-orange-400">247K</p><p className="text-xs text-muted-foreground">Active Groups</p></CardContent></Card>
      </div>
      ...
      <div className="rounded-xl bg-gradient-to-br from-indigo-900/40 to-violet-900/40 border border-indigo-500/30 p-4">
        <p className="font-black text-sm mb-1">Social Groups — Activate Now</p>
        ...
        <div className="grid grid-cols-2 gap-2">
          <Button
            ...
            onClick={() => { setActive(true); toast.success("Social Groups activated!"); }}
          >
            <Zap className="h-4 w-4 mr-2" />{active ? "Active" : "Activate"}
          </Button>
          <Button variant="outline" onClick={() => toast.info("Opening Social Groups docs...")}>
