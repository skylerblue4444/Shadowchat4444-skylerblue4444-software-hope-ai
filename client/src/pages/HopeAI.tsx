...
type VoiceMode = "hands_free" | "guided" | "admin" | "market" | "companion" | "unhinged";
...
const voiceModeOptions: Array<{ value: VoiceMode; label: string; description: string; tone: string }> = [
  { value: "unhinged", label: "Unhinged", description: "Fast route-everything voice, spoken replies, casino/game routing, and kill-switch-safe beta actions.", tone: "border-fuchsia-400/40 bg-fuchsia-400/10 text-fuchsia-100" },
  { value: "hands_free", label: "Hands-free", description: "Run supported actions and navigation without touching every screen manually.", tone: "border-cyan-400/30 bg-cyan-400/10 text-cyan-100" },
  { value: "guided", label: "Guided", description: "Plan first, then execute after the operator reviews IDs and context.", tone: "border-blue-400/30 bg-blue-400/10 text-blue-100" },
  { value: "market", label: "Market", description: "Localize Hope AI for USA, China-ready, and global friends-market growth.", tone: "border-emerald-400/30 bg-emerald-400/10 text-emerald-100" },
  { value: "admin", label: "Admin", description: "Control reviews, trust checks, and blocked action explanations.", tone: "border-amber-400/30 bg-amber-400/10 text-amber-100" },
  { value: "companion", label: "Companion", description: "Community, friends, dating-wave, and creator-support tone.", tone: "border-pink-400/30 bg-pink-400/10 text-pink-100" },
];

const voiceCommandPresets = ["Hope, unhinged mode voice everything", "Hope, open casino", "Hope, play slots beta", "Hope, show production readiness", "Hope, open settings", "Hope, open trading", "Hope, open friends market", "Hope, run full platform boost"];
...
function controlTone(status: string) {
  if (status.includes("live_db") || status.includes("db_backed")) return "border-emerald-400/30 bg-emerald-400/10 text-emerald-100";
  if (status.includes("paper") || status.includes("provider")) return "border-amber-400/30 bg-amber-400/10 text-amber-100";
  if (status.includes("active")) return "border-cyan-400/30 bg-cyan-400/10 text-cyan-100";
  return "border-slate-400/30 bg-slate-400/10 text-slate-200";
}
...
export default function HopeAI() {
  ...
  const [spokenResponse, setSpokenResponse] = useState("Hope AI voice layer is online. Say: unhinged mode voice everything, open casino, go to marketplace, open livestream, post an update, tip creator, like post, show wallet, or open trading.");
  ...
  const runVoiceCommand = (command = voiceText) => {
    ...
    voiceCommand.mutate({ command: trimmed, market, mode: voiceMode, execute: true, content: actionText, postId: numericPostId, targetUserId: numericTargetUserId, amount: numericAmount, coin });
  };
      ...
      <div className="mx-auto max-w-7xl space-y-6 px-4 py-6 sm:px-6 lg:px-8">
          ...
          <div className="relative grid gap-6 lg:grid-cols-[1.45fr_0.55fr]">
            <div className="space-y-5">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <Badge className="border-fuchsia-400/30 bg-fuchsia-400/10 text-fuchsia-200">Unhinged mode</Badge>
                <Badge className="border-emerald-400/30 bg-emerald-400/10 text-emerald-200">Kill-switch gated</Badge>
              </div>
              <div>
                <h1 className="max-w-4xl text-4xl font-black tracking-tight sm:text-6xl">Hope AI Mission Control now drives SkyCoin4444 in unhinged voice-everything mode.</h1>
                <p className="mt-4 max-w-3xl text-base leading-7 text-slate-300 sm:text-lg">This page is no longer a static AI card. It is a voice-command operator layer for marketplace listings, creator tips, likes, casino/game routing, livestream rooms, dating/community waves, bilingual market positioning, admin reviews, settings, paper/day-trade surfaces, and beta crypto-wallet trust logic.</p>
              </div>
        ...
        <section className="grid gap-6 lg:grid-cols-[0.95fr_1.05fr]">
          <Card className="border-cyan-500/20 bg-slate-950/80 text-white shadow-xl shadow-cyan-950/10">
            ...
            <CardContent className="space-y-4">
              <div className="rounded-2xl border border-cyan-400/20 bg-cyan-400/5 p-4"><div className="mb-2 flex flex-wrap items-center gap-2"><Badge className="border-fuchsia-400/30 bg-fuchsia-400/10 text-fuchsia-100">Mode: {voiceModeOptions.find((item) => item.value === voiceMode)?.label}</Badge><Badge className="border-red-400/30 bg-red-400/10 text-red-100">Money kill switches stay on</Badge></div><p className="text-sm leading-6 text-cyan-100">{spokenResponse}</p>{lastRoute && <p className="mt-2 text-xs text-slate-400">Last resolved route: <span className="font-mono text-cyan-200">{lastRoute}</span></p>}</div>
              <div className="space-y-2"><Label htmlFor="voice-command">Say or type a command</Label><div className="flex gap-2"><Input id="voice-command" value={voiceText} onChange={(event) => setVoiceText(event.target.value)} className="border-white/10 bg-white/5 text-white" placeholder="Hope, unhinged mode voice everything" /><Button className="bg-cyan-500 text-black hover:bg-cyan-400" onClick={() => runVoiceCommand()} disabled={voiceCommand.isPending}><Play className="h-4 w-4" /></Button></div></div>
              <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-3">{voiceModeOptions.map((option) => <button key={option.value} onClick={() => { setVoiceMode(option.value); setVoiceText(option.value === "unhinged" ? "Hope, unhinged mode voice everything" : `Hope, ${option.label.toLowerCase()} mode`); }} className={`rounded-2xl border p-3 text-left transition ${voiceMode === option.value ? option.tone : "border-white/10 bg-white/[0.03] text-slate-300 hover:border-cyan-400/30"}`}><p className="font-black">{option.label}</p><p className="mt-1 line-clamp-2 text-xs leading-5 opacity-80">{option.description}</p></button>)}</div>
              <div className="grid gap-2 sm:grid-cols-2">{voiceCommandPresets.map((command) => <Button key={command} variant="outline" className="justify-start border-white/10 bg-white/[0.03] text-left text-xs text-slate-200 hover:bg-fuchsia-500/10" onClick={() => runVoiceCommand(command)} disabled={voiceCommand.isPending}><Zap className="mr-2 h-3.5 w-3.5 text-fuchsia-200" />{command}</Button>)}</div>
              <div className="grid gap-2 sm:grid-cols-4"><Button variant="outline" className="border-cyan-500/30 bg-cyan-500/10 text-cyan-100 hover:bg-cyan-500/20" onClick={startListening} disabled={isListening || voiceCommand.isPending}>{isListening ? <MicOff className="mr-2 h-4 w-4" /> : <Mic className="mr-2 h-4 w-4" />}{isListening ? "Listening" : "Listen"}</Button><Button variant="outline" className="border-fuchsia-500/30 bg-fuchsia-500/10 text-fuchsia-100 hover:bg-fuchsia-500/20" onClick={() => runVoiceCommand("Hope, unhinged mode voice everything")} disabled={voiceCommand.isPending}><BrainCircuit className="mr-2 h-4 w-4" />Arm Unhinged</Button><Button variant="outline" className="border-violet-500/30 bg-violet-500/10 text-violet-100 hover:bg-violet-500/20" onClick={() => { setMuted(!muted); toast.info(!muted ? "Spoken output muted" : "Spoken output enabled"); }}><Volume2 className="mr-2 h-4 w-4" />{muted ? "Muted" : "Voice On"}</Button><Button variant="outline" className="border-emerald-500/30 bg-emerald-500/10 text-emerald-100 hover:bg-emerald-500/20" onClick={() => setAutoNavigate(!autoNavigate)}><Navigation className="mr-2 h-4 w-4" />{autoNavigate ? "Auto Route" : "Preview"}</Button></div>
        ...
        <section className="rounded-[2rem] border border-cyan-500/20 bg-gradient-to-br from-slate-950 via-cyan-950/20 to-slate-950 p-6 text-white">
          <div className="mb-5 flex flex-col justify-between gap-4 lg:flex-row lg:items-end">
            <div>
              <Badge className="mb-3 border-cyan-400/30 bg-cyan-400/10 text-cyan-200"><Globe2 className="mr-1 h-3.5 w-3.5" /> Market + Control Readiness</Badge>
              <h2 className="text-3xl font-black tracking-tight">China, USA, friends-market, settings, hands-free, and day-trade are voice-addressable.</h2>
              <p className="mt-2 max-w-4xl text-sm leading-6 text-slate-300">Hope AI can route these surfaces now and labels every surface as DB-backed, local UI, paper-beta, or provider-gated so unhinged mode moves fast without pretending regulated execution is live.</p>
            </div>
            <Button className="border-0 bg-cyan-500 font-black text-black hover:bg-cyan-400" onClick={() => runVoiceCommand("Hope, unhinged mode voice everything for China USA friends market settings and day trading")} disabled={voiceCommand.isPending}><Languages className="mr-2 h-4 w-4" /> Voice Full Market Stack</Button>
          </div>
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
            {marketControls.map((control) => <button key={control.key} onClick={() => runVoiceCommand(control.voice)} className="group rounded-2xl border border-white/10 bg-white/[0.03] p-4 text-left transition hover:border-cyan-400/40 hover:bg-cyan-500/10"><div className="mb-3 flex items-start justify-between gap-3"><div><p className="font-black text-white">{control.label}</p><p className="mt-1 font-mono text-[11px] text-cyan-200">{control.route}</p></div><ArrowRight className="h-4 w-4 text-slate-500 transition group-hover:translate-x-1 group-hover:text-cyan-200" /></div><Badge className={controlTone(control.status)}>{readinessLabel(control.status)}</Badge><p className="mt-3 text-xs leading-5 text-slate-400">{control.promise}</p><p className="mt-3 text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500">{control.voice}</p></button>)}
          </div>
          <div className="mt-4 rounded-2xl border border-amber-400/20 bg-amber-400/5 p-4 text-sm leading-6 text-amber-100">Day-trade, futures, copy trading, payments, casino payouts, and crypto transfers remain kill-switch/provider-gated. The controls are wired for routing, paper/test flows, audit labels, and operator review before any live regulated execution.</div>
        </section>
        ...
        <section className="grid gap-4 md:grid-cols-3"><MiniPanel icon={Languages} title="China / USA / friends-market logic" body="U.S., global, China-ready, and friends-market copy lives in the orchestration layer so the same Hope AI action can explain its market context." /><MiniPanel icon={Globe2} title="Full-product routing" body="Marketplace, casino, game center, day-trade, settings, dating, livestream, wallet, admin, analytics, AI tools, and Sky Blue IT surfaces are now commandable from one page." /><MiniPanel icon={CheckCircle2} title="Auditable kill-switch autonomy" body="Voice and quick actions log planned, completed, and blocked outcomes instead of silently guessing risky money, trading, casino, or moderation steps." /></section>
      </div>
    </div>
  );
}
