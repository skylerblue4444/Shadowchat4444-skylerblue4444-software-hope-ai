...
export default function ShadowWeb3Identity() {
  const [active, setActive] = useState(false);
  return (
    <div className="space-y-5">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-black">Web3 Identity</h1>
          <p className="text-sm text-muted-foreground">Comprehensive Web3 identity with ENS, DID, verifiable credentials, and reputation</p>
        </div>
        <Badge className="bg-indigo-600 text-white shrink-0">Your Web3 Self</Badge>
      </div>
      <div className="grid grid-cols-4 gap-2">
        <Card className="border-border/50 text-center"><CardContent className="py-3 px-2"><p className="font-black text-lg text-indigo-400">847K</p><p className="text-xs text-muted-foreground">Identities Created</p></CardContent></Card>
        <Card className="border-border/50 text-center"><CardContent className="py-3 px-2"><p className="font-black text-lg text-green-400">47 types</p><p className="text-xs text-muted-foreground">Credentials</p></CardContent></Card>
      ...
      <div className="rounded-xl bg-gradient-to-br from-indigo-900/40 to-violet-900/40 border border-indigo-500/30 p-4">
        <p className="font-black text-sm mb-1">Web3 Identity — Activate Now</p>
        ...
        <div className="grid grid-cols-2 gap-2">
          <Button
            ...
            onClick={() => { setActive(true); toast.success("Web3 Identity activated!"); }}
          >
            <Zap className="h-4 w-4 mr-2" />{active ? "Active" : "Activate"}
          </Button>
          <Button variant="outline" onClick={() => toast.info("Opening Web3 Identity docs...")}>
