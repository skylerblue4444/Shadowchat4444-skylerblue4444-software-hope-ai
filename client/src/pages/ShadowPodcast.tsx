...
export default function ShadowPodcast() {
  const [active, setActive] = useState(false);
  return (
    <div className="space-y-5">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-black">ShadowPodcast</h1>
          <p className="text-sm text-muted-foreground">Web3 podcast platform with NFT episodes, listener rewards, and creator monetization</p>
        </div>
        <Badge className="bg-indigo-600 text-white shrink-0">Podcast Platform</Badge>
      </div>
      <div className="grid grid-cols-4 gap-2">
        <Card className="border-border/50 text-center"><CardContent className="py-3 px-2"><p className="font-black text-lg text-orange-400">84K</p><p className="text-xs text-muted-foreground">Podcasts</p></CardContent></Card>
        ...
        <Card className="border-border/50 text-center"><CardContent className="py-3 px-2"><p className="font-black text-lg text-blue-400">Yes</p><p className="text-xs text-muted-foreground">NFT Episodes</p></CardContent></Card>
        <Card className="border-border/50 text-center"><CardContent className="py-3 px-2"><p className="font-black text-lg text-orange-400">84 SKY/hr</p><p className="text-xs text-muted-foreground">Listener Rewards</p></CardContent></Card>
      </div>
      ...
      <div className="rounded-xl bg-gradient-to-br from-indigo-900/40 to-violet-900/40 border border-indigo-500/30 p-4">
        <p className="font-black text-sm mb-1">ShadowPodcast — Activate Now</p>
        ...
        <div className="grid grid-cols-2 gap-2">
          <Button
            ...
            onClick={() => { setActive(true); toast.success("ShadowPodcast activated!"); }}
          >
            <Zap className="h-4 w-4 mr-2" />{active ? "Active" : "Activate"}
          </Button>
          <Button variant="outline" onClick={() => toast.info("Opening ShadowPodcast docs...")}>
