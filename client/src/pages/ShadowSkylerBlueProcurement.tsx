...
import { Monitor, Package, ShoppingBag, BarChart3, Share2, User, Printer, ShieldCheck, CheckCircle, Zap, TrendingUp } from "lucide-react";
...
const DATA: Record<string, { icon: string; title: string; desc: string; stats: {label:string;value:string;color:string}[]; features: string[]; accent: string }> = {
  ShadowSkylerBlueNOC: {
    icon: "🖥️", title: "Network Operations Center", accent: "text-blue-400",
    ...
    features: ["24/7 network monitoring","Proactive alerting","Bandwidth analysis","Device health tracking","Incident ticketing","Monthly NOC reports"],
  },
  ShadowSkylerBlueAssetMgmt: {
    icon: "📦", title: "IT Asset Management", accent: "text-amber-400",
    ...
    features: ["Hardware inventory","Software license tracking","Warranty management","Depreciation tracking","Disposal workflow","Audit-ready reports"],
  },
  ShadowSkylerBlueProcurement: {
    icon: "🛒", title: "IT Procurement", accent: "text-green-400",
    ...
    features: ["Multi-vendor sourcing","Volume discount negotiation","Purchase order management","Vendor relationship mgmt","Budget tracking","Lifecycle planning"],
  },
  ShadowSkylerBluePowerBI: {
    icon: "📊", title: "IT Analytics & Reporting", accent: "text-purple-400",
    ...
    features: ["Real-time dashboards","Automated reports","Cost allocation","Trend analysis","Executive summaries","Custom KPI tracking"],
  },
  ShadowCryptoSocial: {
    icon: "🌐", title: "Crypto Social Network", accent: "text-cyan-400",
    ...
    features: ["Earn tokens for posting","Tip creators in crypto","NFT profile pictures","Decentralized feed","DAO content moderation","Cross-chain identity"],
  },
  ShadowWeb3Profile: {
    icon: "👤", title: "Web3 Identity Profile", accent: "text-violet-400",
    ...
    features: ["ENS/Unstoppable domains","Verifiable credentials","Cross-chain portfolio","On-chain reputation","SBT soul-bound tokens","Privacy controls"],
  },
  ShadowSkylerBluePrint: {
    icon: "🖨️", title: "Managed Print Services", accent: "text-slate-400",
    ...
    stats: [
      ...
      { label: "Cost/Page",       value: "/usr/bin/bash.008",color: "text-green-400"  },
      { label: "Toner Deliveries",value: "Auto",  color: "text-orange-400" },
    ],
    features: ["Fleet management","Auto toner delivery","Cost-per-page billing","Print security","Usage reporting","Remote diagnostics"],
  },
  ShadowSkylerBlueSecurity2: {
    icon: "🔒", title: "Cybersecurity Services", accent: "text-red-400",
    ...
    features: ["Penetration testing","Phishing simulation","Security awareness training","Incident response","Dark web monitoring","Cyber insurance consulting"],
  },
};

export default function ShadowSkylerBlueProcurement() {
  const d = DATA["ShadowSkylerBlueProcurement"];
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-xl bg-muted/50 border border-border/50 p-4 text-center">
        ...
        <Button className="w-full h-10 bg-indigo-600 text-white border-0 font-black" onClick={() => toast.success("Request sent — we'll contact you within 1 hour!")}>
