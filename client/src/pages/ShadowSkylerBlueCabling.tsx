...
export default function ShadowSkylerBlueCabling() {
  const [active, setActive] = useState(false);
  return (
    <div className="space-y-5">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-black">Network Cabling</h1>
          <p className="text-sm text-muted-foreground">Structured cabling installation and management for Cat6A, fiber, and data centers</p>
        </div>
        <Badge className="bg-indigo-600 text-white shrink-0">Cabling</Badge>
      </div>
      <div className="grid grid-cols-4 gap-2">
        <Card className="border-border/50 text-center"><CardContent className="py-3 px-2"><p className="font-black text-lg text-green-400">8.4M ft</p><p className="text-xs text-muted-foreground">Cables Installed</p></CardContent></Card>
        ...
        <Card className="border-border/50 text-center"><CardContent className="py-3 px-2"><p className="font-black text-lg text-blue-400">25yr</p><p className="text-xs text-muted-foreground">Warranty</p></CardContent></Card>
        <Card className="border-border/50 text-center"><CardContent className="py-3 px-2"><p className="font-black text-lg text-orange-400">TIA-568</p><p className="text-xs text-muted-foreground">Standards</p></CardContent></Card>
      </div>
      ...
      <div className="rounded-xl bg-gradient-to-br from-indigo-900/40 to-violet-900/40 border border-indigo-500/30 p-4">
        <p className="font-black text-sm mb-1">Network Cabling — Activate Now</p>
        ...
        <div className="grid grid-cols-2 gap-2">
          <Button
            ...
            onClick={() => { setActive(true); toast.success("Network Cabling activated!"); }}
          >
            <Zap className="h-4 w-4 mr-2" />{active ? "Active" : "Activate"}
          </Button>
          <Button variant="outline" onClick={() => toast.info("Opening Network Cabling docs...")}>
