...
export default function ShadowCryptoMomentum() {
  const [active, setActive] = useState(false);
  return (
    <div className="space-y-5">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-black">Momentum Trading</h1>
          <p className="text-sm text-muted-foreground">AI-powered momentum trading that identifies and rides strong price trends</p>
        </div>
        <Badge className="bg-indigo-600 text-white shrink-0">Momentum Bot</Badge>
      </div>
      <div className="grid grid-cols-4 gap-2">
        <Card className="border-border/50 text-center"><CardContent className="py-3 px-2"><p className="font-black text-lg text-pink-400">8247</p><p className="text-xs text-muted-foreground">Signals/Day</p></CardContent></Card>
        ...
        <Card className="border-border/50 text-center"><CardContent className="py-3 px-2"><p className="font-black text-lg text-blue-400">Plus 4.7pct</p><p className="text-xs text-muted-foreground">Avg Return/Signal</p></CardContent></Card>
        <Card className="border-border/50 text-center"><CardContent className="py-3 px-2"><p className="font-black text-lg text-orange-400">12</p><p className="text-xs text-muted-foreground">Timeframes</p></CardContent></Card>
      </div>
      ...
      <div className="rounded-xl bg-gradient-to-br from-indigo-900/40 to-violet-900/40 border border-indigo-500/30 p-4">
        <p className="font-black text-sm mb-1">Momentum Trading — Activate Now</p>
        ...
        <div className="grid grid-cols-2 gap-2">
          <Button
            ...
            onClick={() => { setActive(true); toast.success("Momentum Trading activated!"); }}
          >
            <Zap className="h-4 w-4 mr-2" />{active ? "Active" : "Activate"}
          </Button>
          <Button variant="outline" onClick={() => toast.info("Opening Momentum Trading docs...")}>
