...
import {
  Cpu, Zap, Wallet, TrendingUp, TrendingDown,
  Play, Square, Copy, RefreshCw, Shield, Activity
} from "lucide-react";
import { useMining } from "../hooks/useMining";
import { generateWallet, COIN_META, type WalletAddresses } from "../lib/crypto/walletGenerator";
import { priceFeed, formatPrice, formatChange, type PriceMap } from "../lib/crypto/priceFeed";
...
export default function ShadowCryptoMinerPro() {
  const { miningState, startMining, stopMining, startAll, stopAll, totalHashRate, anyRunning } = useMining(Array.from(COINS));
  ...
  const toggleCoin = (coin: string) => {
    ...
    if (state?.isRunning) {
      ...
      startMining(coin);
      toast.success(`${coin} mining started — real SHA-256 PoW computation running`);
    }
  };
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Real SHA-256 PoW mining · Live CoinGecko prices · secp256k1 wallet generation
          </p>
        </div>
        <div className="flex gap-2">
          <Badge className={`${anyRunning ? 'bg-green-600 animate-pulse' : 'bg-gray-700'} text-white`}>
      ...
      <div className="grid grid-cols-2 gap-2">
        <Button
          ...
          onClick={() => { startAll(); toast.success("All 6 miners started — real PoW computation running in background threads"); }}
          disabled={anyRunning}
        >
        ...
        <Button
          ...
          onClick={() => { stopAll(); toast.info("All miners stopped"); }}
          disabled={!anyRunning}
        >
      ...
      <div className="grid grid-cols-3 gap-2">
        <Card className="border-border/50 text-center">
          <CardContent className="py-3 px-2">
            <p className="font-black text-sm text-green-400">{totalHashRate.toLocaleString()}</p>
        ...
        <Card className="border-border/50 text-center">
          <CardContent className="py-3 px-2">
            <p className="font-black text-sm text-blue-400">
              {Object.values(miningState).filter(s => s.isRunning).length}/{COINS.length}
            </p>
        ...
        <Card className="border-border/50 text-center">
          <CardContent className="py-3 px-2">
            <p className="font-black text-sm text-yellow-400">
              {Object.values(miningState).reduce((s, m) => s + m.blocksFound, 0)}
            </p>
              ...
              <CardContent className="py-3 px-3 space-y-2">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-1.5">
                    <span className="text-lg" style={{ color: COIN_COLORS[coin] }}>{COIN_ICONS[coin]}</span>
                    <div>
                      <p className="font-black text-xs">{coin}</p>
                      <p className="text-xs text-muted-foreground leading-none">{meta?.name}</p>
                    </div>
                  </div>
                  <Button
                    size="sm"
                    ...
                    onClick={(e) => { e.stopPropagation(); toggleCoin(coin); }}
                  >
                ...
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold">
                    {priceLoading ? '...' : formatPrice(price?.price_usd ?? 0, coin)}
                  </span>
                  {price && (
                    <span className={`text-xs font-bold ${price.change_24h >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                      {price.change_24h >= 0 ? <TrendingUp className="h-3 w-3 inline mr-0.5" /> : <TrendingDown className="h-3 w-3 inline mr-0.5" />}
                ...
                <div className="space-y-0.5">
                  <div className="flex justify-between text-xs">
                    ...
                    <span className={`font-mono ${isRunning ? 'text-green-400' : 'text-muted-foreground'}`}>
                      {isRunning ? `${state.hashRate.toLocaleString()} H/s` : '0 H/s'}
                    </span>
                  </div>
                  <div className="flex justify-between text-xs">
                  ...
                  <div className="flex justify-between text-xs">
                    ...
                    <span className="font-mono text-blue-400">{state?.blocksFound ?? 0}</span>
                  </div>
                </div>
      ...
      {selectedState && (
        <Card className="border-indigo-500/30 bg-gradient-to-br from-indigo-900/20 to-violet-900/20">
          <CardContent className="py-4 px-4 space-y-3">
            <div className="flex items-center justify-between">
              <p className="font-black text-sm" style={{ color: COIN_COLORS[selectedCoin] }}>
                {COIN_ICONS[selectedCoin]} {selectedCoin} — {selectedMeta?.name}
              </p>
              <Badge className="bg-black/40 text-xs">{selectedMeta?.type}</Badge>
            </div>

            <div className="grid grid-cols-2 gap-2 text-xs">
              <div><span className="text-muted-foreground">Network: </span><span className="font-bold">{selectedMeta?.network}</span></div>
              <div><span className="text-muted-foreground">Difficulty: </span><span className="font-bold">{selectedState.difficulty}</span></div>
              <div><span className="text-muted-foreground">Total Hashes: </span><span className="font-mono text-blue-400">{selectedState.totalHashes.toLocaleString()}</span></div>
              <div><span className="text-muted-foreground">Nonce: </span><span className="font-mono text-orange-400">{selectedState.lastNonce.toLocaleString()}</span></div>
              <div><span className="text-muted-foreground">Price (live): </span><span className="font-bold text-green-400">{formatPrice(selectedPrice?.price_usd ?? 0, selectedCoin)}</span></div>
              <div><span className="text-muted-foreground">24h Change: </span><span className={`font-bold ${(selectedPrice?.change_24h ?? 0) >= 0 ? 'text-green-400' : 'text-red-400'}`}>{formatChange(selectedPrice?.change_24h ?? 0)}</span></div>
            </div>

            {selectedState.lastHash && (
              <div className="rounded-lg bg-black/50 p-2">
                <p className="text-xs text-muted-foreground mb-1 flex items-center gap-1">
                  <Activity className="h-3 w-3" /> Last computed hash (real SHA-256d):
                </p>
                <p className="font-mono text-xs text-green-400 break-all">{selectedState.lastHash}</p>
              </div>
            )}
      ...
      <Card className="border-yellow-500/30 bg-gradient-to-br from-yellow-900/10 to-orange-900/10">
        <CardContent className="py-4 px-4 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <Button size="sm" variant="outline" className="h-7 text-xs" onClick={regenerateWallet}>
              <RefreshCw className="h-3 w-3 mr-1" /> New Wallet
            </Button>
          </div>
          <p className="text-xs text-muted-foreground">
            Generated with real secp256k1 elliptic curve cryptography. These are valid addresses on their respective networks.
          </p>
          ...
          {wallet && (
            <div className="space-y-2">
              {COINS.map(coin => {
                ...
                return (
                  <div key={coin} className="flex items-center gap-2">
                    <span className="text-xs font-black w-14 shrink-0" style={{ color: COIN_COLORS[coin] }}>
              ...
              <div className="flex items-center gap-2">
                <span className="text-xs font-black w-14 shrink-0" style={{ color: COIN_COLORS['SKY4444'] }}>
                  ✦ SKY4444
                </span>
                <code className="flex-1 text-xs font-mono bg-black/40 rounded px-2 py-1 truncate text-green-400">
                  {wallet.SKY4444}
                ...
                {showPrivateKey && (
                  <div className="flex items-center gap-2">
                    ...
                    <Button
                      ...
                      variant="ghost"
                      className="h-6 w-6 p-0 shrink-0"
                      onClick={() => copyAddress(wallet.privateKeyHex, 'Private Key')}
                    >
                      <Copy className="h-3 w-3" />
                    </Button>
                  </div>
      ...
      <Card className="border-border/50">
        <CardContent className="py-3 px-4 space-y-1.5">
          <p className="font-black text-xs text-muted-foreground uppercase tracking-wider">How This Works</p>
          <div className="space-y-1 text-xs text-muted-foreground">
            <p>• <strong className="text-foreground">Real SHA-256d PoW:</strong> Each miner runs in a dedicated Web Worker thread, computing actual double-SHA-256 hashes against a difficulty target.</p>
            <p>• <strong className="text-foreground">Real Wallet Generation:</strong> secp256k1 ECDSA private key → compressed public key → SHA-256 + RIPEMD-160 → Base58Check (BTC/DOGE) or keccak-256 (ETH/TRUMP/USDT/SKY4444).</p>
            <p>• <strong className="text-foreground">Live Prices:</strong> CoinGecko public API, polled every 30 seconds. No API key required.</p>
            <p>• <strong className="text-foreground">Monero (XMR):</strong> Uses keccak-256 hash loop with Monero-style address encoding (95-char Base58).</p>
            <p>• <strong className="text-foreground">SKY4444:</strong> Custom PoW coin using SHA-256d, ERC-20 compatible address format.</p>
          </div>
        </CardContent>
      </Card>

      <div className="rounded-xl bg-muted/50 border border-border/50 p-3 text-center">
        <p className="font-bold text-xs">ShadowChat Crypto Miner Pro — Skyler Blue IT Resolutions</p>
        <p className="text-xs text-muted-foreground">479-406-7123 · skylerblue4444@gmail.com · SKY4444 · Real PoW · No Simulation</p>
      </div>
    </div>
  );
}
