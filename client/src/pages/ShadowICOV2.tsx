...
export default function ShadowICOV2() {
  const [active, setActive] = useState(false);
  return (
    <div className="space-y-5">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-black">ICO V2</h1>
          <p className="text-sm text-muted-foreground">Second-generation ICO platform with KYC, vesting, and investor dashboard</p>
        </div>
        <Badge className="bg-indigo-600 text-white shrink-0">ICO V2</Badge>
      </div>
      <div className="grid grid-cols-4 gap-2">
        <Card className="border-border/50 text-center"><CardContent className="py-3 px-2"><p className="font-black text-lg text-cyan-400">8247</p><p className="text-xs text-muted-foreground">ICOs Hosted</p></CardContent></Card>
        ...
        <Card className="border-border/50 text-center"><CardContent className="py-3 px-2"><p className="font-black text-lg text-blue-400">Full</p><p className="text-xs text-muted-foreground">KYC</p></CardContent></Card>
        <Card className="border-border/50 text-center"><CardContent className="py-3 px-2"><p className="font-black text-lg text-orange-400">Customizable</p><p className="text-xs text-muted-foreground">Vesting</p></CardContent></Card>
      </div>
      ...
      <div className="rounded-xl bg-gradient-to-br from-indigo-900/40 to-violet-900/40 border border-indigo-500/30 p-4">
        <p className="font-black text-sm mb-1">ICO V2 — Activate Now</p>
        ...
        <div className="grid grid-cols-2 gap-2">
          <Button
            ...
            onClick={() => { setActive(true); toast.success("ICO V2 activated!"); }}
          >
            <Zap className="h-4 w-4 mr-2" />{active ? "Active" : "Activate"}
          </Button>
          <Button variant="outline" onClick={() => toast.info("Opening ICO V2 docs...")}>
