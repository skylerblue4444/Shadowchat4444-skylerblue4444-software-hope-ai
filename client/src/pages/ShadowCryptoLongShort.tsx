...
export default function ShadowCryptoLongShort() {
  const [active, setActive] = useState(false);
  return (
    <div className="space-y-5">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-black">Long/Short Ratio</h1>
          <p className="text-sm text-muted-foreground">Real-time long/short ratio tracker across all major exchanges and assets</p>
        </div>
        <Badge className="bg-indigo-600 text-white shrink-0">Long/Short</Badge>
      </div>
      ...
      <div className="rounded-xl bg-gradient-to-br from-indigo-900/40 to-violet-900/40 border border-indigo-500/30 p-4">
        <p className="font-black text-sm mb-1">Long/Short Ratio — Activate Now</p>
        ...
        <div className="grid grid-cols-2 gap-2">
          <Button
            ...
            onClick={() => { setActive(true); toast.success("Long/Short Ratio activated!"); }}
          >
            <Zap className="h-4 w-4 mr-2" />{active ? "Active" : "Activate"}
          </Button>
          <Button variant="outline" onClick={() => toast.info("Opening Long/Short Ratio docs...")}>
