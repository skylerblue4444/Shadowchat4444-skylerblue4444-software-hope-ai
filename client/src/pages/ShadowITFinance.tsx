...
export default function ShadowITFinance() {
  const [active, setActive] = useState(false);
  return (
    <div className="space-y-5">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-black">Finance IT</h1>
          <p className="text-sm text-muted-foreground">PCI-DSS Level 1 compliant IT solutions for banks, credit unions, and fintech companies</p>
        </div>
        <Badge className="bg-indigo-600 text-white shrink-0">PCI-DSS Level 1</Badge>
      </div>
      <div className="grid grid-cols-4 gap-2">
        ...
        <Card className="border-border/50 text-center"><CardContent className="py-3 px-2"><p className="font-black text-lg text-green-400">Level 1</p><p className="text-xs text-muted-foreground">PCI Score</p></CardContent></Card>
        ...
        <Card className="border-border/50 text-center"><CardContent className="py-3 px-2"><p className="font-black text-lg text-orange-400">99.9pct</p><p className="text-xs text-muted-foreground">Fraud Blocked</p></CardContent></Card>
      </div>
      ...
      <div className="rounded-xl bg-gradient-to-br from-indigo-900/40 to-violet-900/40 border border-indigo-500/30 p-4">
        <p className="font-black text-sm mb-1">Finance IT — Activate Now</p>
        ...
        <div className="grid grid-cols-2 gap-2">
          <Button
            ...
            onClick={() => { setActive(true); toast.success("Finance IT activated!"); }}
          >
            <Zap className="h-4 w-4 mr-2" />{active ? "Active" : "Activate"}
          </Button>
          <Button variant="outline" onClick={() => toast.info("Opening Finance IT docs...")}>
