...
import { Heart, Share2, MessageCircle, ShoppingCart, TrendingUp, Zap, Globe2, Filter } from "lucide-react";
import { autoPostScheduler, type AutoPost } from "../lib/autoPost/autoPostEngine";
...
function PostCard({ post, onLike, onShare }: { post: AutoPost; onLike: (id: string) => void; onShare: (id: string) => void }) {
      ...
      <CardContent className="py-3 px-4">
        ...
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <Badge variant="outline" className={`text-xs px-1.5 py-0 ${TYPE_COLORS[post.type] ?? ''}`}>
        ...
        <p className="text-xs text-muted-foreground leading-relaxed mb-2">{post.body}</p>

        {/* Price badge for shop items */}
        {post.price && (
          <div className="flex items-center gap-2 mb-2">
            <span className="text-green-400 font-black text-sm">${post.price}</span>
            {post.discount && (
        ...
        <div className="flex flex-wrap gap-1 mb-2">
          {post.tags.slice(0, 4).map(tag => (
            <span key={tag} className="text-xs text-muted-foreground bg-muted/50 rounded px-1.5 py-0.5">#{tag}</span>
          ))}
        </div>
        ...
        <div className="flex items-center gap-3 pt-1 border-t border-border/30">
          ...
          <button
            onClick={() => { onShare(post.id); toast.success('Shared! +1 SKY4444 earned ✦'); }}
          ...
          {post.shopUrl && (
            <a href={post.shopUrl} className="ml-auto">
              <Button size="sm" className="h-6 text-xs bg-green-600 hover:bg-green-500 text-white border-0 px-2">
                <ShoppingCart className="h-3 w-3 mr-1" /> Buy
              </Button>
            </a>
          )}
          {post.type === 'crypto_signal' && (
            <a href="/dashboard/shadow/day-trade-scream-room" className="ml-auto">
              <Button size="sm" className="h-6 text-xs bg-blue-600 hover:bg-blue-500 text-white border-0 px-2">
...
export default function ShadowSocialFeed() {
  ...
  useEffect(() => {
    ...
    return () => { unsub(); autoPostScheduler.stop(); };
  }, []);
    ...
    <div className="space-y-4 max-w-2xl mx-auto">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground">Auto-posts every 45s · Shop deals · Crypto signals · Mining wins</p>
        </div>
        <div className="flex items-center gap-1.5">
      ...
      <div className="grid grid-cols-4 gap-2">
        {[
          ...
          { label: 'Shop Deals', value: posts.filter(p => p.type === 'shop_item').length, icon: '🛒' },
          { label: 'Signals', value: posts.filter(p => p.type === 'crypto_signal').length, icon: '📊' },
          { label: 'Blocks Found', value: posts.filter(p => p.type === 'block_found').length, icon: '⛏️' },
        ].map(stat => (
      ...
      <div className="flex items-center gap-1.5 overflow-x-auto pb-1">
        ...
        {FILTERS.map(f => (
          ...
          >
            {f === 'USA' ? '🇺🇸 USA' : f === 'China' ? '🇨🇳 China' : f === 'EU' ? '🇪🇺 EU' : f}
          </button>
        ))}
      </div>
      ...
      <div className="flex items-center gap-2 bg-primary/10 border border-primary/20 rounded-lg px-3 py-2">
        ...
        <p className="text-xs text-muted-foreground">
          <span className="font-bold text-foreground">Auto-Post Engine Active</span> — Shop items, crypto signals, and mining wins post automatically every 45 seconds. Share a post to earn <span className="text-indigo-400 font-bold">+1 SKY4444 ✦</span>
        </p>
      </div>
      ...
      <div className="text-center py-4">
        <p className="text-xs text-muted-foreground">ShadowChat Auto-Feed · Skyler Blue IT Resolutions · 479-406-7123</p>
        <p className="text-xs text-muted-foreground">🇺🇸 USA · 🇨🇳 China · 🇪🇺 EU · 🌍 Global</p>
      </div>
    </div>
  );
}
