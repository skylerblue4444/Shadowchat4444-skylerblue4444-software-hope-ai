...
import { Network, Headphones, RefreshCw, ClipboardCheck, Lock, Smartphone, Shield, Activity, CheckCircle, Zap, AlertTriangle } from "lucide-react";
import { Card, CardContent } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { toast } from "sonner";

const PAGE_DATA: Record<string, { icon: string; title: string; desc: string; stats: {label:string;value:string;color:string}[]; features: string[]; color: string }> = {
  ShadowSkylerBlueSDWAN: {
    icon: "🌐", title: "SD-WAN Solutions", color: "text-cyan-400",
    ...
    features: ["Multi-WAN failover","QoS traffic shaping","Zero-trust segmentation","Centralized dashboard","Cloud breakout","Real-time analytics"],
  },
  ShadowSkylerBlueHelpDesk2: {
    icon: "🎧", title: "Help Desk Pro", color: "text-purple-400",
    ...
    features: ["24/7/365 live agents","AI ticket triage","Remote desktop support","Knowledge base","Escalation workflows","SLA reporting"],
  },
  ShadowSkylerBluePatch: {
    icon: "🔧", title: "Patch Management", color: "text-orange-400",
    ...
    features: ["Automated patching","Third-party app updates","Patch compliance reports","Rollback capability","Maintenance windows","CVE tracking"],
  },
  ShadowSkylerBlueAudit: {
    icon: "📋", title: "IT Audit & Assessment", color: "text-yellow-400",
    ...
    features: ["Network vulnerability scan","Security posture assessment","Compliance gap analysis","Hardware inventory","Software license audit","Executive summary report"],
  },
  ShadowSkylerBlueVPN: {
    icon: "🔐", title: "Business VPN", color: "text-green-400",
    ...
    features: ["WireGuard/OpenVPN/AnyConnect","Split tunneling","MFA integration","Kill switch","DNS leak protection","Zero-log policy"],
  },
  ShadowSkylerBlueMDM: {
    icon: "📱", title: "Mobile Device Management", color: "text-blue-400",
    ...
    features: ["iOS/Android/Windows MDM","Remote wipe & lock","App deployment","Policy enforcement","Certificate management","Geo-fencing"],
  },
  ShadowSkylerBlueComplianceMgr: {
    icon: "⚖️", title: "IT Compliance Manager", color: "text-indigo-400",
    ...
    features: ["HIPAA compliance","PCI-DSS controls","SOC 2 Type II","CMMC Level 2","Automated evidence collection","Audit-ready reports"],
  },
  ShadowSkylerBlueSIEM: {
    icon: "🛡️", title: "SIEM & Log Management", color: "text-red-400",
    ...
    features: ["Real-time log aggregation","AI threat detection","Automated alerting","Incident response playbooks","Compliance reporting","Forensic investigation"],
  },
};

export default function ShadowSkylerBlueComplianceMgr() {
  const d = PAGE_DATA["ShadowSkylerBlueComplianceMgr"];
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-xl bg-muted/50 border border-border/50 p-4 text-center">
        ...
        <Button className="w-full h-10 bg-indigo-600 text-white border-0 font-black" onClick={() => toast.success("Request sent — we'll call you within 1 business hour!")}>
          <Zap className="h-4 w-4 mr-2" />Get Free Assessment
        </Button>
      </div>
    </div>
  );
