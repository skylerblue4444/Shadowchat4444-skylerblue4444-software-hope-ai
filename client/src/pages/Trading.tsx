...
export default function Trading() {
  ...
  const [priceHistory, setPriceHistory] = useState(() => generatePriceHistory(0.4821));
  const [orderBook, setOrderBook] = useState(() => generateOrderBook(0.4821));
  const [recentTrades, setRecentTrades] = useState<{ price: string; size: string; side: "buy" | "sell"; time: string }[]>([]);
  ...
  const priceChange = ((currentPrice - priceHistory[0]?.price) / priceHistory[0]?.price * 100).toFixed(2);
  ...
  useEffect(() => {
    intervalRef.current = setInterval(() => {
      setPriceHistory(prev => {
        ...
        const newPrice = Math.max(last + (Math.random() - 0.49) * last * 0.008, 0.1);
        const now = new Date();
  ...
  const handleOrder = () => {
    ...
    const effectivePrice = orderMode === "market" ? currentPrice.toFixed(4) : limitPrice;
    if (!effectivePrice) { toast.error("Enter a limit price"); return; }
    createOrder.mutate({ pair, type: orderType, amount, price: effectivePrice });
  };

  const total = amount && (orderMode === "market" ? currentPrice : parseFloat(limitPrice || "0"))
    ? (parseFloat(amount) * (orderMode === "market" ? currentPrice : parseFloat(limitPrice || "0"))).toFixed(2)
    : "0.00";
          ...
          >
            <CardContent className="py-3 px-4">
              ...
              <div className="flex items-center gap-2 mt-0.5">
                <p className="font-bold text-sm">{m.price < 1 ? m.price.toFixed(4) : m.price.toLocaleString()}</p>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-4 gap-4">
        ...
        <div className="xl:col-span-3 space-y-4">
              ...
              <div className="flex flex-wrap items-center justify-between gap-4">
                <div>
                  <div className="flex items-center gap-3">
                    ...
                    <Badge className={`${isUp ? "bg-green-500/10 text-green-400 border-green-500/20" : "bg-red-500/10 text-red-400 border-red-500/20"}`}>
                      {isUp ? <ArrowUpRight className="h-3 w-3 mr-1" /> : <ArrowDownRight className="h-3 w-3 mr-1" />}
                      {priceChange}%
                    </Badge>
                  </div>
                  <p className="text-sm text-muted-foreground mt-1">{pair} · Live · Updates every 2s</p>
                </div>
                <div className="flex gap-6 text-sm">
                  {[
                    { label: "24h High", value: "0.4950", color: "text-green-400" },
                    { label: "24h Low", value: "0.4612", color: "text-red-400" },
                    { label: "24h Vol", value: "2.4M", color: "text-foreground" },
                  ].map((stat) => (
                    <div key={stat.label}>
            ...
            <CardContent className="pt-4 pb-2">
              <ResponsiveContainer width="100%" height={280}>
                <LineChart data={priceHistory}>
                  <XAxis dataKey="time" tick={{ fontSize: 10, fill: "#6b7280" }} tickLine={false} axisLine={false} interval={9} />
                  <YAxis domain={["auto", "auto"]} tick={{ fontSize: 10, fill: "#6b7280" }} tickLine={false} axisLine={false} tickFormatter={(v) => v.toFixed(3)} width={55} />
                  <Tooltip
                    contentStyle={{ background: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderRadius: "8px", fontSize: "12px" }}
                    formatter={(v: number) => [v.toFixed(4), "Price"]}
                  />
                  <ReferenceLine y={priceHistory[0]?.price} stroke="#374151" strokeDasharray="3 3" />
                  <Line type="monotone" dataKey="price" stroke={parseFloat(priceChange) >= 0 ? "#22c55e" : "#ef4444"} strokeWidth={2} dot={false} />
                </LineChart>
              </ResponsiveContainer>
            </CardContent>
          </Card>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Card className="border-border/50">
              <CardHeader className="pb-2"><CardTitle className="text-sm">Order Book</CardTitle></CardHeader>
              <CardContent>
                <div className="grid grid-cols-2 gap-1 text-xs text-muted-foreground mb-2 px-1">
                  <span>Price (USDC)</span><span className="text-right">Size</span>
                </div>
                {orderBook.asks.map((ask, i) => (
                  <div key={i} className="grid grid-cols-2 gap-1 text-xs py-0.5 px-1 hover:bg-red-500/5 rounded">
                    <span className="text-red-400 font-mono">{ask.price}</span>
                    <span className="text-right text-muted-foreground font-mono">{parseInt(ask.size).toLocaleString()}</span>
                  </div>
                ))}
                <div className="my-2 py-2 px-1 bg-muted/30 rounded text-center">
                  <span className={`text-sm font-bold ${isUp ? "text-green-400" : "text-red-400"}`}>{currentPrice.toFixed(4)}</span>
                </div>
                {orderBook.bids.map((bid, i) => (
                  <div key={i} className="grid grid-cols-2 gap-1 text-xs py-0.5 px-1 hover:bg-green-500/5 rounded">
                    <span className="text-green-400 font-mono">{bid.price}</span>
                    <span className="text-right text-muted-foreground font-mono">{parseInt(bid.size).toLocaleString()}</span>
                  </div>
                ))}
              </CardContent>
            </Card>
            ...
            <Card className="border-border/50">
              ...
              <CardContent>
                <div className="grid grid-cols-3 gap-1 text-xs text-muted-foreground mb-2 px-1">
                  <span>Price</span><span className="text-center">Size</span><span className="text-right">Time</span>
                </div>
                <AnimatePresence initial={false}>
                  {recentTrades.slice(0, 12).map((trade, i) => (
                    <motion.div key={`${trade.time}-${i}`} initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} className="grid grid-cols-3 gap-1 text-xs py-0.5 px-1">
                      <span className={`font-mono ${trade.side === "buy" ? "text-green-400" : "text-red-400"}`}>{trade.price}</span>
                      <span className="text-center text-muted-foreground font-mono">{parseInt(trade.size).toLocaleString()}</span>
                      <span className="text-right text-muted-foreground font-mono">{trade.time}</span>
                    </motion.div>
                  ))}
                </AnimatePresence>
                {recentTrades.length === 0 && <p className="text-xs text-muted-foreground text-center py-4">Waiting for trades...</p>}
              </CardContent>
            </Card>
          </div>
        </div>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex gap-2">
                {(["market", "limit"] as const).map((mode) => (
                  <button key={mode} onClick={() => setOrderMode(mode)} className={`flex-1 py-1.5 text-xs rounded-md border transition-colors capitalize ${orderMode === mode ? "border-blue-500 text-blue-400 bg-blue-500/10" : "border-border text-muted-foreground hover:border-blue-500/30"}`}>{mode}</button>
                ))}
              </div>
              <div>
                <label className="text-xs text-muted-foreground mb-1 block">Pair</label>
                <select className="w-full h-9 rounded-md border border-input bg-background px-3 text-sm focus:outline-none focus:ring-2 focus:ring-ring" value={pair} onChange={(e) => setPair(e.target.value)}>
                  {PAIRS.map(p => <option key={p} value={p}>{p}</option>)}
                </select>
              </div>
              <div>
                <label className="text-xs text-muted-foreground mb-1 block">Amount ({pair.split("/")[0]})</label>
                <Input type="number" placeholder="0.00" value={amount} onChange={(e) => setAmount(e.target.value)} className="font-mono" />
                <div className="flex gap-1 mt-1.5">
                  {["25%", "50%", "75%", "100%"].map((pct) => (
                    <button key={pct} className="flex-1 text-xs py-1 rounded bg-muted hover:bg-muted/80 text-muted-foreground transition-colors">{pct}</button>
                  ))}
                </div>
              </div>
              {orderMode === "limit" && (
                <div>
                  ...
                  <Input type="number" placeholder={currentPrice.toFixed(4)} value={limitPrice} onChange={(e) => setLimitPrice(e.target.value)} className="font-mono" />
                </div>
              )}
              {orderMode === "market" && (
                <div className="flex items-center justify-between text-xs p-2 bg-muted/30 rounded-lg">
                  <span className="text-muted-foreground">Market Price</span>
                  <span className={`font-mono font-semibold ${isUp ? "text-green-400" : "text-red-400"}`}>{currentPrice.toFixed(4)} USDC</span>
                </div>
              )}
