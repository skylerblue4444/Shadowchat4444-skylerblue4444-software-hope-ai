...
const curatedSupplierCatalog = [
  {
    ...
    description:
      "Curated supplier-import placeholder for ring light, tabletop tripod, mic, and product-demo props. Replace with verified DHgate, Alibaba, or direct supplier rows before live fulfillment.",
    category: "Streaming",
    ...
    reviewSummaryJson: JSON.stringify([
      {
        source: "curated",
        text: "Demo review summary only; import real reviews from approved supplier files or APIs.",
      },
    ]),
    specsJson: JSON.stringify({
      channel: "livestream commerce",
      reviewPolicy: "admin approval required",
    }),
    price: "44.00",
  ...
  },
  {
    ...
    description:
      "Curated entertainment-commerce drop for RGB desk lights, keyboard accessories, and Space Quest streamer setups. Real supplier rows can override this seeded entry.",
    category: "Gaming",
    supplierName: "Admin Curated Supplier Desk",
    supplierCountry: "Global",
    sourceUrl: null,
    imageUrlsJson: JSON.stringify([
      "https://images.unsplash.com/photo-1587829741301-dc798d83add3?w=600&h=600&fit=crop",
    ]),
    reviewSummaryJson: JSON.stringify([
      {
        source: "curated",
        text: "Demo popularity only; verify supplier reviews before public fulfillment.",
      },
    ]),
    specsJson: JSON.stringify({
      questTieIn: "Space Quest",
      reviewPolicy: "admin approval required",
    }),
    price: "31.20",
  ...
  },
  {
    ...
    description:
      "Curated catalog item for power banks, mini lights, and creator accessories that help video streamers stay live. Import real supplier cost and availability before fulfillment.",
    ...
    reviewSummaryJson: JSON.stringify([
      {
        source: "curated",
        text: "Seeded trend card; not a confirmed live supplier order history.",
      },
    ]),
    specsJson: JSON.stringify({
      streamUse: "portable power",
      reviewPolicy: "admin approval required",
    }),
    price: "28.00",
...
export function hydrateSupplierCatalogItem(
  row:
    | typeof supplierCatalogItems.$inferSelect
    | (typeof curatedSupplierCatalog)[number]
) {
  return {
    ...row,
    ...
    sourceLabel:
      row.providerStatus === "live_api"
        ? "Live supplier API"
        : row.providerStatus === "admin_import"
          ? "Admin imported supplier data"
          : "Curated demo seed",
    reviewLabel:
      row.reviewStatus === "approved"
        ? "Admin approved"
        : row.reviewStatus === "queued"
...
export async function getSupplierProviderStatus() {
  const providerStatus = supplierProviders.map(provider => {
    ...
    const configured = envName
      ...
      : provider === "admin_import" || provider === "private_supplier";
    return {
      provider,
      displayName:
        provider === "dhgate"
      ...
      mode:
        configured && envName
          ...
          : provider === "admin_import"
            ? "admin_upload"
            : provider === "private_supplier"
              ? "manual_supplier"
              : "not_configured",
      message:
        configured && envName
          ? "Server-side provider ID is configured for live supplier calls."
          : provider === "admin_import"
            ? "Admins can import supplier catalog rows immediately."
            : provider === "private_supplier"
              ? "Private supplier rows can be imported and reviewed by admins."
              : "Set the provider API ID environment variable and credentials before live sync.",
    };
  });
  ...
  if (!db)
    return {
      providerStatus,
      persistedSyncs: [],
      dataMode: "curated_fallback" as const,
    };
  const persistedSyncs = await db
    .select()
    .from(supplierProviderSyncs)
    .orderBy(desc(supplierProviderSyncs.updatedAt))
    .limit(20);
  return {
    providerStatus,
    persistedSyncs,
    dataMode: persistedSyncs.length
      ? ("mixed_provider_state" as const)
      : ("admin_import_ready" as const),
  };
}

export async function listSupplierCatalog(input?: {
  search?: string;
  category?: string;
  limit?: number;
}) {
  ...
  if (!db)
    return curatedSupplierCatalog
      .slice(0, limit)
      .map(hydrateSupplierCatalogItem);

  const filters = [
    eq(supplierCatalogItems.reviewStatus, "approved"),
    ne(supplierCatalogItems.providerStatus, "removed"),
  ];
  if (input?.category && input.category !== "All" && input.category !== "all")
    filters.push(eq(supplierCatalogItems.category, input.category));
  if (input?.search) {
    ...
    const rows = await db
      ...
      .where(and(...filters))
      .orderBy(desc(supplierCatalogItems.updatedAt))
      .limit(200);
    const filtered = rows
      .filter(row =>
        `${row.title} ${row.description} ${row.category} ${row.supplierName}`
          .toLowerCase()
          .includes(lowered)
      )
      .slice(0, limit);
    return (
      filtered.length ? filtered : curatedSupplierCatalog.slice(0, limit)
    ).map(hydrateSupplierCatalogItem);
  }
  ...
  const rows = await db
    ...
    .orderBy(desc(supplierCatalogItems.updatedAt))
    .limit(limit);
  return (rows.length ? rows : curatedSupplierCatalog.slice(0, limit)).map(
    hydrateSupplierCatalogItem
  );
}
...
export async function importSupplierCatalogRows(
  ...
  const db = await getDb();
  if (!db)
    throw new Error("Database is required for supplier catalog imports.");
  ...
  for (const row of rows) {
    const [created] = await db
      ...
      .values({
        ...
        description:
          row.description ??
          "Admin-imported supplier product. Verify source URL, images, reviews, supplier cost, and shipping before approval.",
        ...
        compareAtPrice:
          row.compareAtPrice === undefined
            ? undefined
            : normalizeMoney(row.compareAtPrice),
...
export async function syncSupplierProviderCatalog(
  provider: Exclude<SupplierProvider, "admin_import" | "private_supplier">,
  query: string,
  adminId: number
) {
  ...
  const db = await getDb();
  if (!db)
    throw new Error("Database is required for supplier provider sync state.");
  if (!apiId) {
    ...
    throw new Error(
      `${provider} supplier API is not configured. Add the server-side provider API ID and credentials before live sync.`
    );
  }

  await db
    .insert(supplierProviderSyncs)
    .values({
      provider,
      displayName:
        provider === "dhgate"
  ...
  return {
    ...
    payload,
    nextStep:
      "Map the provider payload into supplier catalog import rows after confirming the provider response contract for this account.",
  };
}

export async function createSupplierOrderRequest(
  userId: number,
  ...
  const platformMargin =
    Math.max(0, subtotal - estimatedSupplierCost) + serviceFee;
  const providers = Array.from(
    new Set(sanitizedItems.map(item => item.provider).filter(Boolean))
  );
  const provider =
    providers.length === 1 && providers[0] !== "mixed"
      ? (providers[0] as SupplierOrderProvider)
      : "mixed";

  const [created] = await db
    .insert(supplierOrderRequests)
    .values({
      requesterId: userId,
      provider,
  ...
  return {
    ...
    total: normalizeMoney(subtotal + serviceFee),
    reviewNotice:
      "Supplier order request queued for admin review. No live supplier order or payment capture occurs until approval and provider confirmation.",
  };
}

export async function listSupplierOrderRequests(input?: {
  userId?: number;
  limit?: number;
}) {
  const db = await getDb();
  ...
  const rows = input?.userId
    ? await db
        .select()
        .from(supplierOrderRequests)
        .where(eq(supplierOrderRequests.requesterId, input.userId))
        .orderBy(desc(supplierOrderRequests.createdAt))
        .limit(limit)
    : await db
        .select()
        .from(supplierOrderRequests)
        .where(
          or(
            eq(supplierOrderRequests.orderStatus, "queued"),
            eq(supplierOrderRequests.orderStatus, "admin_review"),
            eq(supplierOrderRequests.orderStatus, "approved")
          )
        )
        .orderBy(desc(supplierOrderRequests.createdAt))
        .limit(limit);
  return rows.map(row => ({
    ...row,
    cartItems: parseJson<SupplierCartItemInput[]>(row.cartItemsJson, []),
  }));
}

export async function updateSupplierOrderRequestStatus(input: {
  orderId: number;
  orderStatus:
    | "queued"
    | "admin_review"
    ...
    | "rejected"
    | "cancelled";
  paymentStatus?: "not_charged" | "quote_sent" | "held" | "paid" | "refunded";
  adminNote?: string;
}) {
  const db = await getDb();
  if (!db)
    throw new Error("Database is required for supplier order admin controls.");
  await db
    .update(supplierOrderRequests)
    .set({
      orderStatus: input.orderStatus,
      paymentStatus: input.paymentStatus,
      adminNote: input.adminNote,
    })
    .where(eq(supplierOrderRequests.id, input.orderId));
  return { success: true };
}
