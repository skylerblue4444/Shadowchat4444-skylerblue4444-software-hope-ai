...
export const charityRouter = router({
  listCauses: publicProcedure.query(async () => {
  ...
  joinGameSession: publicProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      return {
        success: true,
        session,
        message: `Joined ${input.gameType} for cause! TRUMP deducted.`,
      };
    }),
  ...
  getImpactMetrics: publicProcedure.query(async () => {
    const totalRaised = MOCK_CAUSES.reduce(
      (sum, c) => sum + c.currentAmount,
      0
    );
    const totalImpact = MOCK_CAUSES.reduce(
      (sum, c) => sum + calculateImpact(c.currentAmount, c.trumpMultiplier),
      0
    );
    return {
      ...
      topCause: MOCK_CAUSES.sort(
        (a, b) => b.currentAmount - a.currentAmount
      )[0],
      multiplierAvg:
        MOCK_CAUSES.reduce((sum, c) => sum + c.trumpMultiplier, 0) /
        MOCK_CAUSES.length,
    };
  }),
  ...
  mintStoryNFT: publicProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const nft = {
        ...
        rarity:
          input.coAuthorIds && input.coAuthorIds.length > 1
            ? "legendary"
            : "rare",
        mintedAt: new Date(),
      ...
      return {
        success: true,
        nft,
        message: "Impact Story NFT minted! View in your vault.",
      };
    }),

  getMultiAgentLog: publicProcedure.query(async () => {
