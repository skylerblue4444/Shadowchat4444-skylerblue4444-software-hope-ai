...
const demoStreams = [
  ...
  },
  {
    ...
    topic:
      "YouTube-style creator feed, private marketplace safety, and creator monetization.",
    channelType: "youtube",
...
function decoratePostContent(input: {
  kind: string;
  title?: string;
  content: string;
  mediaUrl?: string;
  ...
  const header = input.title
    ? `[${input.kind.toUpperCase()}] ${input.title}`
...
export const liveSocialRouter = router({
  listStreams: publicProcedure
    .input(
      z
        .object({
          status: z
            .enum(["scheduled", "live", "ended", "flagged", "all"])
            .default("all"),
    ...
    .query(async ({ input }) => {
      ...
      if (input?.status && input.status !== "all") {
        return query
          .where(eq(liveStreams.status, input.status))
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (!db)
        throw new TRPCError({
          ...
          message: "Database is required for livestream hosting.",
        });
  ...
  endStream: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (!db)
        throw new TRPCError({
          code: "PRECONDITION_FAILED",
          message: "Database is required for livestream controls.",
        });
      await db
        .update(liveStreams)
        .set({ status: "ended" })
        .where(
          and(
            ...
            eq(liveStreams.hostId, ctx.user.id)
          )
        );
      return { success: true };
    }),
  ...
  tipStream: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (!db)
        throw new TRPCError({
          code: "PRECONDITION_FAILED",
          message: "Database is required for livestream tipping.",
        });
      ...
      const tip = await multiCoinService.tip(
        ctx,
        stream.hostId,
        input.amount,
        input.coin as Coin,
        input.memo ?? `Livestream tip for ${stream.title}`
      );
      await db
        .update(liveStreams)
        .set({
          tipPool: sql`${liveStreams.tipPool} + ${input.amount.toFixed(8)}`,
        })
        .where(eq(liveStreams.id, input.streamId));
  ...
  createFeedPost: protectedProcedure
    .input(
      z.object({
        kind: z
          .enum(["profile", "youtube", "livestream", "marketplace", "status"])
          .default("status"),
  ...
  listFeed: publicProcedure
    ...
    .query(async ({ input }) =>
      getFeedPosts(input?.limit ?? 25, input?.offset ?? 0)
    ),
  ...
  interactWithPost: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (!db)
        throw new TRPCError({
          code: "PRECONDITION_FAILED",
          message: "Database is required for feed interactions.",
        });
      const [post] = await db
        .select()
        .from(posts)
      ...
      if (input.type === "tip") {
        if (!input.amount || input.amount <= 0)
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "Tip amount must be positive.",
          });
        await multiCoinService.tip(
          ctx,
          post.userId,
          input.amount,
          input.coin as Coin,
          input.content ?? "Profile feed creator tip"
        );
      }
