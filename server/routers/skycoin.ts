...
async function creditBetaBalance(
  userId: number,
  coin: (typeof supportedCoins)[number],
  amount: number,
  memo: string
) {
  ...
  if (!db)
    throw new TRPCError({
      ...
      message: "Database is required for beta reward claims.",
    });
  ...
  await db.transaction(async tx => {
    if (coin === "SKY4444") {
      await tx
        .update(users)
        .set({ balance: sql`${users.balance} + ${amountText}` })
        .where(eq(users.id, userId));
    } else {
      const [existing] = await tx
        .select()
        .from(holdings)
        .where(and(eq(holdings.userId, userId), eq(holdings.asset, coin)))
        .limit(1);
      if (existing) {
        await tx
          .update(holdings)
          .set({ amount: sql`${holdings.amount} + ${amountText}` })
          .where(eq(holdings.id, existing.id));
      } else {
        await tx
          .insert(holdings)
          .values({ userId, asset: coin, amount: amountText });
      }
    }
    ...
    await recordSettlementEntry(tx, {
      ...
      audit: {
        ...
        supplyEvent: "airdrop",
        providerGate:
          "internal beta ledger only; no withdrawal or external payment rail executed",
      },
    });
  ...
  getEconomyStatus: protectedProcedure.query(async ({ ctx }) => {
    ...
    return {
      ...
      productionGate: {
        ...
        reason:
          "Real custody, withdrawals, payments, and external chain operations stay disabled until secure provider credentials and compliance checks are configured.",
      },
      betaCapabilities: [
        "wallet ledger",
        "creator tips",
  ...
  claimBetaReward: protectedProcedure
    .input(
      z.object({
        ...
        reason: z.string().max(120).default("daily-live-feature-reward"),
      })
    )
    .mutation(async ({ ctx, input }) =>
      creditBetaBalance(
        ctx.user.id,
        input.coin,
        betaRewards[input.coin],
        `Beta ${input.reason} claim`
      )
    ),
  ...
  recordSupplyEvent: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (!db)
        throw new TRPCError({
          code: "PRECONDITION_FAILED",
          message: "Database is required for supply event controls.",
        });
