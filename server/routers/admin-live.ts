...
import {
  getPendingBeginnerPlusBusinessIntents,
  updateBeginnerPlusBusinessReviewStatus,
} from "../lib/beginner-plus-business-intents";
import {
  getPendingReviewEntries,
  updateSettlementReviewStatus,
...
const beginnerPlusReviewStatusSchema = z.enum([
  "queued",
  "approved",
  "rejected",
]);
  ...
  overview: adminProcedure.query(async () => {
    ...
    if (!db) {
      return {
        ...
        recentSupplyEvents: [],
        betaNotice:
          "Database not configured; admin overview is in demo-safe mode.",
      };
    }
    ...
    const [
      ...
      db
        .select()
        .from(tokenSupplyEvents)
        .orderBy(desc(tokenSupplyEvents.createdAt))
        .limit(10),
    ]);
    ...
    return {
      ...
      recentSupplyEvents,
      betaNotice:
        "Admin analytics are backed by production database tables while external money movement remains gated.",
    };
  }),
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (!db)
        throw new TRPCError({
          ...
          message: "Database is required for admin user controls.",
        });
      ...
      if (typeof input.verified === "boolean")
        patch.verified = input.verified ? 1 : 0;
      if (input.balance) patch.balance = input.balance;
      if (Object.keys(patch).length === 0)
        throw new TRPCError({
          code: "BAD_REQUEST",
          message: "No admin update fields supplied.",
        });
  ...
  moderationQueue: adminProcedure.query(async () => {
    ...
    const [flaggedPosts, datingBlocks] = await Promise.all([
      db
        .select()
        .from(posts)
        .where(eq(posts.sentiment, "flagged"))
        .orderBy(desc(posts.updatedAt))
        .limit(25),
      db
        .select()
        .from(datingActions)
        .where(eq(datingActions.action, "block"))
    ...
    .query(async ({ input }) => {
      ...
      if (!db) {
        return {
          ...
          pendingCount: 0,
          betaNotice:
            "Database not configured; settlement review is unavailable in demo-safe mode.",
        };
      }
      ...
      return {
        ...
        betaNotice:
          "Settlement review covers beta ledger, paper trade, provider-gated, and test-mode entries before any real-world money movement is enabled.",
      };
    }),
    ...
    .mutation(async ({ ctx, input }) => {
      if (input.reviewStatus === "queued" && !input.adminNote) {
        throw new TRPCError({
          code: "BAD_REQUEST",
          message:
            "A note is required when returning a settlement entry to queued review.",
        });
      }
      const db = await getDb();
      if (!db)
        throw new TRPCError({
          code: "PRECONDITION_FAILED",
          message: "Database is required for settlement review.",
        });
      const entry = await updateSettlementReviewStatus(
        db,
        input.id,
        input.reviewStatus,
        input.adminNote
    ...
    .query(async ({ input }) => {
      ...
      if (!db) {
        return {
          ...
          pendingCount: 0,
          betaNotice:
            "Database not configured; Beginner Plus business review is unavailable in demo-safe mode.",
        };
      }
      const pendingIntents = await getPendingBeginnerPlusBusinessIntents(
        db,
        input?.limit ?? 25
      );
      return {
        ...
        betaNotice:
          "Beginner Plus review covers guided posts, profile trust, offers, creator monetization, and partner paths before any public claim, payment, identity verification, or production monetization is enabled.",
      };
    }),
    ...
    .mutation(async ({ ctx, input }) => {
      if (input.reviewStatus === "queued" && !input.adminNote) {
        throw new TRPCError({
          code: "BAD_REQUEST",
          message:
            "A note is required when returning a Beginner Plus business intent to queued review.",
        });
      }
      const db = await getDb();
      if (!db)
        throw new TRPCError({
          code: "PRECONDITION_FAILED",
          message: "Database is required for Beginner Plus business review.",
        });
      const intent = await updateBeginnerPlusBusinessReviewStatus(
        db,
        input.id,
        input.reviewStatus,
        ctx.user.id,
      ...
      await writeAudit(
        ctx.user.id,
        "beginner_plus_business.review.updated",
        {
          beginnerPlusBusinessIntentId: input.id,
          reviewStatus: input.reviewStatus,
          adminNote: input.adminNote,
        },
        intent?.userId
