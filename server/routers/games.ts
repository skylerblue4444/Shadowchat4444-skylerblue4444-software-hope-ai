...
function settleGame(
  ...
  if (session.game === "blackjack") {
    ...
    outcome = {
      ...
      dealer,
      natural,
      result: rewardMultiplier > 0 ? "win" : "house",
    };
  }
  ...
  if (session.game === "roulette") {
    ...
    outcome = {
      ...
      color,
      pick,
      result: rewardMultiplier > 0 ? "win" : "miss",
    };
  }
  ...
  if (session.game === "slots") {
    ...
    const reels = [
      numberFromDigest(hex, 0, symbols.length) - 1,
      numberFromDigest(hex, 8, symbols.length) - 1,
      numberFromDigest(hex, 16, symbols.length) - 1,
    ].map(i => symbols[i]);
  ...
  if (session.game === "crash") {
    ...
    const cashoutAt = Math.max(
      1.01,
      Math.min(Number(params?.cashoutAt ?? 1.5), 10)
    );
    rewardMultiplier = cashoutAt <= crashAt ? cashoutAt : 0;
    outcome = {
      crashAt,
      cashoutAt,
      result: rewardMultiplier > 0 ? "cashed" : "crashed",
    };
  }
  ...
  if (session.game === "dice") {
    const threshold = Math.max(
      2,
      Math.min(Number(params?.threshold ?? 50), 98)
    );
    rewardMultiplier =
      baseRoll >= threshold ? Number((98 / (101 - threshold)).toFixed(2)) : 0;
    outcome = {
      roll: baseRoll,
      threshold,
      result: rewardMultiplier > 0 ? "win" : "miss",
...
export const gamesRouter = router({
  catalog: publicProcedure.query(() => ({
    ...
    infrastructure: {
      fairness:
        "Each session commits a serverSeedHash before play and reveals the serverSeed after settlement for beta verification.",
      wallet:
        "Rewards are routed through the beta multi-coin ledger reward hook; production wagering can later swap this to smart-contract escrow.",
      audit:
        "Every round stores action, nonce, reward, and seed hash in the session audit trail.",
    },
  })),
  ...
  startSession: protectedProcedure
    .input(
      z.object({
        game: gameSchema,
        wager: z.number().min(0).max(100000).default(1),
        coin: coinSchema.default("SKY4444"),
        clientSeed: z.string().min(3).max(120).default("sky-client"),
      })
    )
    .mutation(({ ctx, input }) => {
      ...
      return {
        ...
        serverSeedHash: session.serverSeedHash,
        betaNotice:
          "Beta entertainment session created. This is not public regulated gambling infrastructure.",
      };
    }),
  ...
  playRound: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const walletReward =
        result.reward > 0
          ? await multiCoinService.mine(
              ctx,
              session.coin,
              Math.min(25, Math.max(1, result.reward)),
              `Beta ${session.game} reward for session ${session.id}`
            )
          : null;
        ...
        await db.transaction(async tx => {
          await recordSettlementEntry(tx, {
            ...
            audit: {
              ...
              walletReward,
              providerGate:
                "casino session is beta entertainment; no external gambling rail executed",
            },
          });
        });
      }
      return {
        ...
        reveal: {
          ...
          serverSeedHash: session.serverSeedHash,
          verifyHash: sha256(session.serverSeed),
        },
        audit: session.audit,
      };
    }),
  ...
  auditTrail: protectedProcedure.query(({ ctx }) => ({
    sessions: Array.from(sessions.values())
      .filter(session => session.userId === ctx.user.id)
      .slice(-25)
      .map(session => ({
        id: session.id,
        game: session.game,
