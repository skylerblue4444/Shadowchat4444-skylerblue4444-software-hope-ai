...
export const datingRouter = router({
    ...
    .input(
      z.object({
        ...
        seeking: z
          ...
          .min(2)
          .max(120)
          .default("networking, dating, creator collabs"),
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (!db)
        throw new TRPCError({
          ...
          message: "Database is required for dating profiles.",
        });
  ...
  discover: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      return db
        ...
        .where(
          and(
            eq(datingProfiles.isVisible, 1),
            ne(datingProfiles.userId, ctx.user.id)
          )
        )
        .orderBy(
          desc(datingProfiles.compatibilityScore),
  ...
  react: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (!db)
        throw new TRPCError({
          code: "PRECONDITION_FAILED",
          message: "Database is required for dating reactions.",
        });
      ...
      const [reciprocal] = await db
        ...
        .where(
          and(
            ...
            or(
              eq(datingActions.action, "like"),
              eq(datingActions.action, "superlike")
            )
          )
        )
        .limit(1);
      ...
      return {
        success: true,
        matched: Boolean(
          reciprocal && ["like", "superlike"].includes(input.action)
        ),
      };
    }),
  ...
  getMatches: protectedProcedure.query(async ({ ctx }) => {
    ...
    const outgoing = await db
      ...
      .where(
        and(
          eq(datingActions.actorId, ctx.user.id),
          or(
            eq(datingActions.action, "like"),
            eq(datingActions.action, "superlike")
          )
